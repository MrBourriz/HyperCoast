...
class Map(leafmap.Map):
    ...
    # Read the EnMAP dataset
     ...
     source["reflectance"] = source["reflectance"].where(source["reflectance"] != nodata)

     # Create the image
     image = enmap_to_image(source, wavelengths=wavelengths, method=method)

     # Set colormap to None for multi-band data
     if isinstance(wavelengths, list) and len(wavelengths) > 1:
        colormap = None

     # Ensure wavelengths are in a list
     if isinstance(wavelengths, int):
        wavelengths = [wavelengths]

     # Default indexes for RGB or single-band visualization
     if indexes is None:
        if isinstance(wavelengths, list) and len(wavelengths) == 1:
            indexes = [1]
        else:
    ...
    # Update metadata and band names
